...
(function (wp) {
  ...
  registerBlockType('modfarm/book-page-tax', {
    apiVersion: 2,
    title: 'Book Page: Related (Taxonomy)',
    category: 'widgets',
    icon: 'index-card',
    ...
    edit: function (props) {
      ...
      const taxSource = attributes['tax-source'] || 'series';
      const displayLayout = attributes['display-layout'] || 'grid';
      const isHorizontal = displayLayout === 'horizontal';

      // Card-level flags with sensible defaults (mirrors multi-tax-format)
      ...
      const excludeCurrent = attributes['exclude-current'] !== false; // default true

      // Format taxonomy (book-format) for filtering
      ...
      return el(
        ...
        el(InspectorControls, {},
          ...
          // Book Filters — differs from multi-tax: current-book taxonomy instead of term picker
          el(PanelBody, { title: __('Book Filters', 'modfarm'), initialOpen: true },
            el(SelectControl, {
              ...
              onChange: (val) => setAttributes({ 'tax-source': val })
            }),
            el(ToggleControl, {
              label: __('Exclude Current Book', 'modfarm'),
              checked: !!excludeCurrent,
              onChange: (val) => setAttributes({ 'exclude-current': !!val })
            }),
            el(SelectControl, {
          ...
          // Card Layout & Styles (global vs local overrides) — shared with multi-tax-format
          ...
          // Display Toggles (title/series/buttons/sample override) — shared with multi-tax-format
